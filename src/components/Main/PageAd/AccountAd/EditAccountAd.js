...
const EditAccountAd = ({ navigation, route }) => {
  const { nv } = route.params;
  const { colors } = useTheme();
  const { avatar, iconCamera } = styles;
  const [ht, setHT] = useState(nv.NV_HOTEN);
  const [gt, setGT] = useState(nv.NV_GIOITINH);
  const [ns, setNS] = useState(nv.NV_NGAYSINH);
  const [dc, setDC] = useState(nv.NV_DIACHI);
  const [m, setM] = useState(nv.NV_MAIL);
  //const [sdt, setSDT] = useState(nv.NV_SODIENTHOAI);

  function edit() {
    if (!ht || !gt || !ns || !dc || !m) {
      Alert.alert('Thông báo', 'Vui lòng nhập đầy đủ thông tin');
    } else {
      fetch('http://192.168.195.111:80/WebAPI/public/editad', {
        method: 'POST',
        headers: {
        ...
        .then(() => {
          navigation.navigate('LoginAd', { nv });
        });
    }
  }
      ...
      >
        <View>
          {pickedImagePath !== '' && (
            <Image
              source={{
                uri: `http://10.233.3.87/healthapp_web/image/nhanvien/${nv.NV_HINHANH}`,
              }}
              resizeMode="contain"
              style={avatar}
            />
